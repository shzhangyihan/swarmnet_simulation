...
#define BLOOM_FILTER_SIZE_BYTE (BLOOM_FILTER_SIZE / BYTE_SIZE)
#define BLOOM_FILTER_1_OFFSET (RND_CHECK_OFFSET + ID_MAX_LEN)
#define BLOOM_FILTER_2_OFFSET (BLOOM_FILTER_1_OFFSET + BLOOM_FILTER_SIZE_BYTE)
...
class Default_program : public Kilobot {
    ...
    bool id_bloom_filter[BLOOM_FILTER_SIZE];
    bool checker_bloom_filter[BLOOM_FILTER_SIZE];
    float bloom_filter_timer;
    std::hash<int> hash_func;

   public:
    void clear_bloom_filter() {
        for (int i = 0; i < BLOOM_FILTER_SIZE; i++) {
            id_bloom_filter[i] = false;
            checker_bloom_filter[i] = false;
        }
        bloom_filter_timer = get_local_time();
    }
    ...
    void message_rx(packet_t packet, situated_sensing_t sensing) {
        ...
        if (get_local_time() - bloom_filter_timer > BLOOM_FILTER_TIME_OUT) {
            clear_bloom_filter();
        }
        ...
        if (id_size == 0) {
            // do nothing here
        } else if (src_id == id && src_id_size == id_size) {
            ...
            // check bloom filter
            int id_hash = get_hash(id);
            int checker_hash = get_hash(collide_checker);
            if (check_bloom_filter(packet.payload + BLOOM_FILTER_1_OFFSET,
                                   id_hash)) {
                if (check_bloom_filter(packet.payload + BLOOM_FILTER_2_OFFSET,
                                       checker_hash)) {
                    // self, ignore
                } else {
                    std::cout << "wow bloom filter worked" << std::endl
                              << std::flush;
                    id_collided();
                }
            }
    ...
    bool message_tx(packet_t* packet) {
        if (id_size > 0) {
            ...
            for (int i = 0; i < BLOOM_FILTER_SIZE; i++) {
                ...
                if (id_bloom_filter[i]) filter_byte += 1;
                if (index == BYTE_SIZE - 1) {
                    packet->payload[BLOOM_FILTER_1_OFFSET + offset] =
                        filter_byte;
                    filter_byte = 0;
                }
            }
            ...
            for (int i = 0; i < BLOOM_FILTER_SIZE; i++) {
                ...
                int index = i % BYTE_SIZE;
                filter_byte = filter_byte << 1;
                if (checker_bloom_filter[i]) filter_byte += 1;
                if (index == BYTE_SIZE - 1) {
                    packet->payload[BLOOM_FILTER_2_OFFSET + offset] =
                        filter_byte;
                    filter_byte = 0;
                }
            }
