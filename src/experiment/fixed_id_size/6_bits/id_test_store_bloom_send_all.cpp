...
#define ID_MAX_LEN 4
// #define ID_SRC_SIZE_OFFSET 1  // leave one for pkt size
...
#define TRUE_ID_OFFSET (BLOOM_FILTER_OFFSET + BLOOM_FILTER_SIZE / BYTE_SIZE)

#define PACKET_LENGTH (TRUE_ID_OFFSET + 1)  // one for true ID
   ...
   public:
    ...
    void message_rx(packet_t packet, situated_sensing_t sensing) {
        ...
        for (int i = 0; i < num_pkt; i++) {
            ...
            packet_t cur_packet;
            memcpy(cur_packet.payload, packet.payload + pkt_offset,
                   PACKET_LENGTH);

            int src_id = read_id(cur_packet.payload);
            int src_id_size = cur_packet.payload[ID_SRC_SIZE_OFFSET];
            int ttl = cur_packet.payload[TTL_OFFSET];
            ...
            if (ttl == 0) {
                ...
                add_bloom_buffer(cur_packet);
            }

            add_to_forward(cur_packet);
    ...
    bool message_tx(packet_t* packet) {
        ...
        int pkt_offset = 1;
        set_id(packet->payload + pkt_offset, id);
        packet->payload[ID_SRC_SIZE_OFFSET + pkt_offset] = id_size % 256;
        packet->payload[TTL_OFFSET + pkt_offset] = MAX_TTL;
        packet->payload[TRUE_ID_OFFSET + pkt_offset] = node_id;
        ...
        for (int i = 0; i < BLOOM_FILTER_SIZE; i++) {
            ...
            if (index == BYTE_SIZE - 1) {
                packet->payload[BLOOM_FILTER_OFFSET + offset + pkt_offset] =
                    filter_byte;
                filter_byte = 0;
            }
        }
        ...
        packet_t cur_pkt;
        memcpy(cur_pkt.payload, packet->payload + pkt_offset, PACKET_LENGTH);
        add_to_cache(cur_pkt);

        for (int i = 0; i < f_buf.size; i++) {
            int pkt_offset = 1 + (i + 1) * PACKET_LENGTH;
            memcpy(packet->payload + pkt_offset, f_buf.buf[i].payload,
                   PACKET_LENGTH);
            add_to_cache(f_buf.buf[i]);
        }
        clear_tx_buf();
        ...
        //     int src_id = read_id(packet->payload + offset);
        //     int src_node_id = packet->payload[offset + TRUE_ID_OFFSET];
    ...
    void set_id(unsigned char* in_msg_payload, int in_id) {
        int remain = in_id;
        int index = 0;
        int offset;
        ...
        while (remain != 0) {
            in_msg_payload[offset + index] = remain % 256;
            remain = remain / 256;
            index = index + 1;
        }
    }

    int read_id(unsigned char* in_msg_payload) {
        int out_id = 0;
        int index = 0;
        int offset;
        int out_id_size;
        ...
        for (int i = out_id_byte_size - 1; i >= 0; i--) {
            out_id = out_id * 256 + (uint64_t)in_msg_payload[offset + i];
        }
