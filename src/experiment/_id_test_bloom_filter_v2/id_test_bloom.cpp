...
class Default_program : public Kilobot {
   ...
   public:
    ...
    void update_check_filter() {
        int filter_counter[BLOOM_FILTER_SIZE];
        // std::cout << "in checker" << std::endl << std::flush;
        ...
        // std::cout << "in checker 2" << std::endl << std::flush;
        ...
        for (int i = 0; i < BLOOM_FILTER_SIZE; i++) {
            if (my_bloom_filter.filter[i]) {
                check_filter.filter[i] = true;
                filter_true_count++;
            } else {
                if (buff_size == 0)
                    check_filter.filter[i] = false;
                else {
                    int random_num = rand() % (buff_size * BLOOM_FILTER_SIZE);
        ...
        // std::cout << "in checker 3" << std::endl << std::flush;

        // std::cout << node_id << " true count from " << old_filter_true_count
        //           << " to " << filter_true_count << std::endl
        //           << std::flush;
        ...
        check_filter.timer = get_local_time();
    }
    ...
    bool message_tx(packet_t* packet) {
        ...
        if (id_size > 0) {
            ...
            for (int i = 0; i < BLOOM_FILTER_SIZE; i++) {
                ...
                if (check_filter.filter[i]) filter_byte += 1;
    ...
    int new_sample_id(int id_size) {
        ...
        while (true) {
            ...
            if (filter_true_count < (int)pow(2, id_size) &&
                ...
                check_filter.filter[hash_val] == true) {
                continue;
            } else {
                break;
            }
